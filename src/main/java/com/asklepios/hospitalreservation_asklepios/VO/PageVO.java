...
public class PageVO {
    private int startNo;	//현재 페이지의 시작 글번호
    private int endNo;		//현재 페이지의 마지막 글 번호
    private int perPageNum=10;	//한 페이지 당 글 개수
    private Integer page; // Integer 웹에서 받은 페이지 정보(String)가 없으면 null인데 int는 null을 저장할 수 없다. 오류방지
    //현재 페이지 (value값은 문자열이지만 integer를 사용하면 정수형을 객체로 저장)
    private int totalCount;	//전체 게시글 개수
    private int endPage;	//현재 페이지의 표시되는 마지막 페이지
    private int startPage;	//현재 페이지의 표시되는 첫번째 페이지
